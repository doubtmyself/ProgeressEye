...
- `appConfig/policies.adFreeModeGlobal == true`이면 모든 계정에서 Free 플랜도 보상형 광고를 건너뛴다.
- 전역 광고 비표시 모드가 `true`일 때는 설정 화면에서 Pro 구독 진입 UI를 숨긴다.
...
- 광고 비표시 모드는 `appConfig/policies.adFreeModeGlobal` 플래그로 제어하며, 모든 계정에 동일하게 적용된다.
- `adFreeModeGlobal`은 클라이언트 앱에서 직접 변경하지 않고, 관리자(Firebase Console 또는 서버 함수)만 변경한다.
